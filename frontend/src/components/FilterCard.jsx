...
const FilterCard = () => {
  ...
  return (
    <div style={{
      width: '100%', backgroundColor: '#fdfaf4', border: '1px solid #e0d5c0',
      borderRadius: '14px', padding: '1.25rem', boxShadow: '0 1px 4px rgba(74,103,65,0.07)',
    }}>
      ...
      {filterData.map((group) => (
        <div key={group.name} style={{ marginBottom: '1.25rem' }}>
          <p style={{ fontSize: '0.78rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#4a6428', margin: '0 0 0.5rem' }}>
            {group.filterType}
          </p>
            ...
            {group.array.map((item) => {
              ...
              return (
                <label key={item.value} style={{
                  ...
                  padding: '4px 6px', borderRadius: '6px', transition: 'background-color 0.12s ease',
                ...
                >
                  <input type="checkbox" checked={isChecked} onChange={() => toggle(group.name, item.value)}
                    style={{ width: '15px', height: '15px', accentColor: '#4a6741', cursor: 'pointer', flexShrink: 0 }}
                  />
                  <span style={{ fontSize: '0.85rem', color: isChecked ? '#2c3e1f' : '#4a3f2f', fontWeight: isChecked ? 600 : 400 }}>
                    {item.label}
                  </span>
                </label>
              );
